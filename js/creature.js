Creature = {
	...
	touch: function (me,to,distance)
	{
		...
		if(distance<=hip)
		{
			...
			{
				if(to_cr.escape_time>0 && my_cr.mode!=="sex")
				{
					return false;
				}
				else if(to_cr.sex_desire<to_cr.sex_threshold)
				{
	...
	create_animals: function ()
	{
		...
		Creature.timers["create_animals"] = setTimeout("Creature.create_animals()",Math.floor((Math.random()*40000)+1000));
	...
	create_plants: function ()
	{
		...
		Creature.timers["create_plants"] = setTimeout("Creature.create_plants()",Math.floor((Math.random()*30000)+1000));
	},
